...
Boards are generated by splitting the grid recursively, so the partition always
tiles it exactly, and each clue is read off the piece it lands in — which makes
the generated tiling itself a valid solution.

Controls: drag out a rectangle, click one again to take it back. `N` clear,
